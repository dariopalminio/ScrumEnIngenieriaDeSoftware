...
Además, por un lado son los propios equipos los que pueden responsabilizarse de DevOps y de su propio pipeline de Continuous Delivery, ya sea trabajando colaborativamente entre los Devs, Testers y Ops o con perfiles de trabajo DevOps.

\begin{figure}[h]
  \centering
  \includegraphics[width=0.99\textwidth]{DevOps-pipeline}
  \caption{DevOps=Dev+Tester+Ops}
  \centering
  \label{fig:DevOps-pipeline} %\ref{fig:DevOps-pipeline}
\end{figure}

Por otro lado, el área de operaciones (o equipo DevOps o de plataforma e infraestructura) no necesita implementar Scrum, pero sí puede llevar en práctica sus valores, ser ágil (con Lean, Kanban, DAD, etc.) y tener en cuenta los ciclos Scrum de los equipos de desarrollo. Este equipo, más que trabajar como auditor, rector y bajo demanda (un modo push) deberían trabajar más en un modo facilitador, líder y como soporte (un modo pull) de los equipos de desarrollo, promoviendo un sistema de autogestión (donde los equipos puedan autogestionar sus sistemas DevOps incluyendo su plataforma e infraestructura).
DevOps es sólo un paso importante para unirse a la cultura general de la colaboración ágil y que debe involucrar a todas las disciplinas en una organización. 
