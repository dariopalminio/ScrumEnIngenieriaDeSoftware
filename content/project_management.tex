...
Bajo el marco Scrum, ayuda al equipo a medir su propio desempeño en el proceso de trabajo para poder hacer cambios basados en hechos. También es un soporte a la gestión de proyectos para poder medir el avance del proyecto en cuanto a impacto de negocio (no a seguir un plan), revisar la productividad de valor y el desempeño\footnote{Las métricas se pueden usar para medir desempeño, pero no se deben utilizar en forma punitiva ni tampoco en sistemas de evaluación de desempeño de empleados.} del equipo en cuanto a madurez. Además hay indicadores de calidad de producto que nos pueden indicar aspectos claves de prestancia y salud de un producto. 

En síntesis, se pueden categorizar las métricas en tres aspectos: proceso de trabajo, calidad de producto e impacto de negocio.

\begin{figure}[h]
  \centering
  \includegraphics[width=0.70\textwidth]{Metrics_KPIs}
  \caption{KPIs.}
  \centering
...
\subsubsection{Unidades de medida relativas}

Para poder usar métricas y medir es necesario unidades de medida. Con Scrum se suelen comenzar a usar unidades relativas en los casos en que se necesita medir de antemano o estimar o cuando se quiere medir algo no determinístico o tangible como el esfuerzo o complejidad. Las más usadas son:
...
La verdadera medida de éxito en Scrum es el incremento de producto que es valioso para el cliente. Pero… ¿Cómo medir qué es valioso para el cliente?
Scrum no prescribe métricas de negocio o indicadores clave de impacto de negocio, aunque es necesario usarlos para saber si estamos entregando valor o generando el impacto deseado. El PO tiene el poder de tomar la decisión última de negocio y necesita datos empíricos para hacerlo. Además, en el proceso de inspección y adaptación, para construir el producto adecuado, el equipo con el PO necesitan observar indicadores de producto, orientarse, decidir y actuar, en busca de maximizar el valor. En esta vía, el PO tiene la posibilidad de usar Business Value Point para estimar valor entregado o hacer una distinción entre historias de usuario de cara a cliente o técnicas (más adelante explicado). Por otro lado también se pueden manejar un conjunto mínimo y suficiente de indicadores de negocio. A estos indicadores se los suele llamar KPI's y en el caso de negocio podemos interpretarlos como indicadores claves de impacto del producto. Algunos son los siguientes.
  ...
  \item {\textbf{Business Value Delivered:} Valor de negocio entregado. Se pueden contabilizar los BV entregados en cada Sprint.
} 
  ...
  \item {\textbf{CSAT:} Customer Satisfaction es el índice de grado de satisfacción de los usuarios o clientes.
}
  ...
  \item {\textbf{Funne:} El Funne o Full Compliance es el promedio de usuarios que terminan de usar nuestra aplicación con éxito.
}
