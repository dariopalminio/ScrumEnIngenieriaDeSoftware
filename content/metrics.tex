...
\begin{enumerate}    
 ...
 \item {\textbf{Delivery Lead Time:} Desde el punto de vista de DevOps se puede medir el Lead Time desde que se hace el primer commit hasta que el código se despliega en producción\footnote{\cite{Humble-2018}}.
  ...
  \begin{figure}[h]
  \centering
  \includegraphics[width=0.99\textwidth]{DevOps-Metrics}
  \caption{DevOps pipeline y sus métricas}
  \centering
  \label{fig:DevOps-Metrics} %\ref{fig:DevOps-Metrics}
  \end{figure}
  \FloatBarrier
...
Algunos son los siguientes.
  ...
  \item {\textbf{NPS:} El Puntaje Neto de Promotores (Net Promoter Score) mide la lealtad general de los clientes hacia un producto o servicio (fidelización). Mide qué probabilidad hay de que el cliente recomiende la compañía y, de forma indirecta, la propensión del consumidor a seguir siéndolo y resistir irse a la competencia. Sus valores varían entre -100 y 100 y es la resta entre el porcentaje de promotores y detractores. Es recomendable que sea mayor a 50.
}

  \begin{figure}[h]
  \centering
  \includegraphics[width=0.99\textwidth]{nps_screenshot}
  \caption{Ejemplo de cálculo de NPS}
  \centering
  \label{fig:nps_screenshot} %\ref{fig:nps_screenshot}
  \end{figure}
  \FloatBarrier
